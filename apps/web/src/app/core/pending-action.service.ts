...
export interface PendingAction {
  id: string;
  /** Ключ действия: id замечания, а для действий вне карточки — любой стабильный ключ («member:<userId>»). */
  remarkId: string;
  /** Подпись решения дословно: «В работу разработчикам», «Закрыть: исправлено»… */
  label: string;
  /** Слово перед подписью в общей полосе; по умолчанию «Решение:», для «Убрали Ивана из проекта» — пусто. */
  record?: string;
