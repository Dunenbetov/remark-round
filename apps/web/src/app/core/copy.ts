...
export const TAGLINE = 'Журнал замечаний: цитата из ТЗ, скрин и решение человека';
...
export const HINT = {
  ...
  card: {
    pm: 'Слева замечание, в центре черновик с цитатой из ТЗ, справа — одна кнопка вашего решения. Клавиши 1–5.',
...
export const TOUR = {
  ...
  pm: {
    steps: [
      ...
      {
        status: 'triaging',
        title: 'Мы готовим разбор',
        text: 'Цитата из ТЗ, факты со скрина, черновик разбора и похожие замечания раунда. Модель ничего не решает — только собирает всё, что нужно для решения.',
        illustration: 'steps',
      },
      {
        status: 'awaiting_pm',
        title: 'Ваше решение — одна кнопка',
        text: 'Слева замечание, в центре черновик, справа пять кнопок тремя группами: это работа · это не работа · нужны данные. Клавиши 1–5, пять секунд на «Отменить».',
        illustration: 'keys',
      },
      {
        status: 'defect',
...
/** «Забыли пароль» (ADR 012): письмо со ссылкой; существование адреса не раскрываем. */
export const FORGOT = {
  pageTitle: 'Восстановление пароля',
  lead: 'Введите e-mail, с которым регистрировались, — пришлём ссылку для смены пароля.',
  email: 'E-mail',
  submit: 'Отправить ссылку',
  sent: (email: string) => `Если адрес ${email} у нас есть, письмо со ссылкой уже в пути. Ссылка действует час.`,
  noMail: 'Почта на этом сервере не настроена — попросите администратора помочь со входом.',
...
export const RESET = {
  ...
  dead: 'Ссылка не действует: прошёл час или её уже использовали.',
...
export const ADMIN = {
  ...
  invited: (email: string) => `Приглашение для ${email} создано — скопируйте ссылку и отправьте сами`,
  invitedMailed: (email: string) => `Письмо со ссылкой ушло на ${email}; на всякий случай ссылка есть и здесь`,
  ...
  linkReadyMailed: 'Письмо с новой ссылкой ушло; на всякий случай она есть и здесь — прежняя больше не работает',
  copied: 'Скопировано',
...
export const TEAM = {
  ...
  addedMemberMailed: (name: string) => `${name} — в проекте, письмо со ссылкой на проект ушло`,
  addedInvitation: (email: string) => `Приглашение для ${email} создано — скопируйте ссылку и отправьте сами`,
  addedInvitationMailed: (email: string) => `Письмо со ссылкой ушло на ${email}; на всякий случай ссылка есть и здесь`,
  invitationsHintMail: 'Письмо со ссылкой уходит само. В проект попадает только тот, кто пришёл по ссылке; регистрация на этот e-mail без ссылки ничего не даёт. Ссылка живёт неделю: потеряли — выпустите новую, прежняя перестанет работать.',
...
export const PROFILE = {
  ...
  notifyHint: 'Одно письмо на всё, что накопилось за несколько минут: решение, дефект в работу, новый кадр для ретеста, закрытие. Приглашений и смены пароля это не касается.',
};
