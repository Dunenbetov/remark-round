 ...
 * По умолчанию показан дифф (иначе «Стало», иначе первый). Кольцо диффа — 2px тил `--rr-accent-2-text` (закон маркера, VISUAL.md),
 * «дышит» один раз при первом показе. Клик по кадру → open(index), страница откроет просмотрщик.
 *
 * Режим «Сравнить» (слайдер Было/Стало с ручкой [role=slider][data-rr-keys], snap к 50%, при близких пропорциях
 * кадров) — после беты: docs/BETA-REVIEW.md, «первые две недели».
 */
@Component({
  ...
  styles: `
    ...
    /* закон маркера: кольцо кадра «Дифф» тилом */
    .stage__frame--diff {
