...
function system(p: LlmParams): string {
  ...
  return [
    'Ты готовишь для руководителя приёмки разбор замечания приёмки веб-проекта (RemarkRound). Решение не за тобой: его принимает человек кнопкой.',
    'Опора — фрагменты пакета документов этого проекта (ТЗ, протокол), факты кадра и другие замечания раунда. Чего нет в опоре, того не существует.',
...
export function systemPrompts(p: LlmParams = llmParams()): Record<'triage' | 'vision' | 'rewrite' | 'explain' | 'judge', string> {
  ...
  return {
    triage: base,
    vision: `${base}\n\nСейчас твоя задача — только факты кадра, без вывода о классе замечания.`,
