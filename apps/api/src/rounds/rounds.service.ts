...
export class RoundsService {
  ...
  async list(ctx: ProjectContext): Promise<RoundSummary[]> {
    ...
    return rounds.map((r) => ({ id: r.id, number: r.number, status: r.status, remarks: r._count.remarks, closedAt: r.closedAt?.toISOString() ?? null }));
  }
  ...
  async create(ctx: ProjectContext, number?: number): Promise<RoundSummary> {
    ...
    return { id: round.id, number: round.number, status: round.status, remarks: 0, closedAt: null };
  }
  ...
  private async summary(roundId: string): Promise<RoundSummary> {
    ...
    return { id: r.id, number: r.number, status: r.status, remarks: r._count.remarks, closedAt: r.closedAt?.toISOString() ?? null };
  }
}
