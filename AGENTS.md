...
ТЗ не судья. Судья — человек. Система собирает улики (чанки пакета документов, скрин, pixel-diff) и имеет право сказать `cannot_tell` / `unspecified`. `unspecified` ≠ change request. Закрытие замечания и превращение в работу разработчика — только `HumanVerdict`. Чужой `projectId` режется SQL, не промптом.
