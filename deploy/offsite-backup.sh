...
#   /backups  — дампы pg_dump сервиса backup (том postgres-backups) → $OFFSITE_REMOTE/postgres
#   /storage  — кадры, документы, диффы (том api-storage)           → $OFFSITE_REMOTE/storage
# rclone sync — зеркало: удалённые на сервере файлы пропадут и в копии, поэтому ретеншн дампов
# (14 дней / 8 недель / 6 месяцев) задаёт сервис backup, а не эта копия. Конфиг remote — через переменные
