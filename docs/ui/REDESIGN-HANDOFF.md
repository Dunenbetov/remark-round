...
- **`pages/remark-card-page.ts` — 1264 строки** (было 915, план требовал ≤700 после выноса компонентов). Шапка, колонка замечания, навигация и сцена вынесены, но страница выросла за счёт режимов и клавиатуры. Если её снова придётся править — сначала вынести блок «Что написал заказчик» и режимы `retest-wait`/`refuse` в `ui/evidence.ts` (компонент запланирован, но не создан: его роль сейчас исполняет разметка внутри страницы).
...
| **Капс.** На карточке капс остался только у трёх заголовков колонок: подпись рельса, «Что написал заказчик», «Что сделать», подписи групп решения и подписи тайлов журнала — 13 обычным регистром | `queue-rail`, `remark-card-page`, `round-tiles` |
