...
- `judge_frames` — проигравшая ветка A/B (модель сама решает по двум кадрам, без диффа). Включается только `RETEST_STRATEGY=llm_only`, для сравнения (`:13-17`, ADR 002).
...
- **Не покрыто тестом:** цикл `rewrite_query` (заглушка модели в тестах его не считает; в отчёте evals с 18.09 есть `rewriteCount` по кейсам, живые цифры — после M1) и пробуждение графа вторым экземпляром API после рестарта: держится на устройстве (чекпоинт в Postgres), отдельной спеки нет.
