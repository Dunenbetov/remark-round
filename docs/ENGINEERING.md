...
9. **Улика — снимок.** `EvidenceCitation` хранит текст цитаты и подпись документа на момент предложения; `chunkId` — живая ссылка с `ON DELETE SET NULL`. Переиндексация ТЗ не отнимает обоснование у принятых и закрытых замечаний.
