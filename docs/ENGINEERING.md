...
9. **История — только дописывается.** Каждый переход оставляет `RemarkStatusChange` в той же транзакции (`transition(...)`, `createNumbered`, `failRun`, `cancelRun`): кто, в какой роли, каким прогоном, короткая пометка. Прямой `remark.update({ status })` вне этих мест — ошибка ревью.
10. **Улика — снимок.** `EvidenceCitation` хранит текст цитаты и подпись документа на момент предложения; `chunkId` — живая ссылка с `ON DELETE SET NULL`. Переиндексация ТЗ не отнимает обоснование у принятых и закрытых замечаний.
...
| `remarks.history.spec` | строка на переход с автором, ролью и прогоном; гонка вердиктов не даёт лишней строки; заказчик — без содержания предложений; `AgentRun` хранит предложение после вердикта |
