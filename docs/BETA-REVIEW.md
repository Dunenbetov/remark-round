...
**Итог 17.09.2026 (агент):** дни 1–6 раздела 7 закрыты, 20 коммитов на `main` (запушены владельцем 17.09 вместе с ADR 013). Сделано: R-B1–R-B3, R-H1, R-H2, R-H4, R-H5, R-M2, R-M6, R-M7, R-L5, R-H3 (бета-лимит), I-1–I-6, A-1, A-2, D-1–D-5, разделы 3.1–3.3. Проверено: `pnpm --filter @remarkround/api test` — 241 тест (два прогона; спека закрытия без кадра один раз флейкнула под нагрузкой и прошла 2/2 отдельно), tsc, сборка mcp и web (894 кБ при пороге 900), `pnpm evals -- --offline` (binding 21/30, faithfulness 30/30, leakage ok — базовая линия режима правил), `docker compose config` без профиля и с ним; стенд пересобран на новый образ. **Владельцу:** ~~аккаунт SMTP и DNS (4.2)~~ (отменено ADR 013), проект в Langfuse Cloud и ключи, `SENTRY_DSN`/`SENTRY_DSN_WEB`, `.env` по чеклисту 6.4, репетиция restore (R-M3), тег `v0.9.0-beta`. Деплой — Railway (ветка `main`, см. `docs/PROD-RAILWAY.md`), `dev` — ветка разработки. На первые две недели: R-M4, R-M5, I-7, R-L1–R-L4, R-L6, R-L7, фоновый импорт (R-H3), слайдер «Сравнить», удаление `admin` из enum.
...
| 17.09 | 3.2 | Шаблон журнала — только `GET /imports/template.*` (фронт качает blob через `ApiService.importTemplate`, статика из `public/` и запись в неё из `make-journal-fixtures` удалены, спека сравнивает только API); `GET /documents/:id` **оставлен** (фронт его не зовёт, но спеки `tenancy.leakage` и `uploads.hygiene` читают документ по id — маршрут описан как контракт для внешних клиентов); даты — `dateRu`/`dayMonthRu` из `format.ts`; `labels.contract.spec` сверяет ключи подписей и enum'ы web с Prisma по тексту исходников; scripts `make:fixtures`/`make:frames`/`make:screenshots`/`rag:eval` |
| 17.09 | 3.3 | `docs/archive/` (README, `PHASES.md`, `presentation/`), ссылки в `AGENTS.md`/`.cursor`/`REMARKROUND.md`/README/DEMO/спеках; `in_dev` убран из доктрины; `fixtures/README.md` без «судьи»; 4 неподключённых скриншота удалены (pngquant на машине нет — сжатие остальных пропущено); `.claude/launch.json` вне git + `launch.example.json`; `TODO(PR7)` снят (слайдер — «первые две недели»); `rr-hint-line` убран из VISUAL.md. `docs/ui/02-remark-card.md` **не восстановлен**: он ссылается на удалённый `reference.html` (старый канон) — карточка описана в VISUAL.md и в самом приложении |
| 17.09 | D-1…D-5 | `seed:remove` (скрипт, режим entrypoint, спека на throwaway-данных — каскад проходит триггер ADR 011); демо-персоны и пароль отдаёт `GET /auth/options` только при `demoLogins` (из бандла убраны `LOGIN.roles`/`LOGIN_EXTRA`, seed берёт персон из `auth/demo-accounts.ts`); `COPY fixtures` в образе сужен до spec/protocol/screenshots; `SEED_ON_START` в образе по умолчанию `false` (compose ставит `true` явно); `.cursor/mcp.json` без пароля (`${env:REMARKROUND_PASSWORD}`) |
| 17.09 | R-H4 | `StorageService.save` проверяет `statfs` (свободно < `STORAGE_MIN_FREE_MB`, 2 ГБ → 507 `storage_full`) и квоту проекта `STORAGE_QUOTA_MB_PER_PROJECT` (2 ГБ, по сумме файлов каталога с кэшем на минуту — размера кадра в БД нет); спека `media.spec`; PROD «Что делать, если» (отдельный диск для томов) |
...
| 17.09 | CI (YAML) | Workflow `ci.yml` не разбирался с 07.09: двоеточие в имени шага «Сборка фронта (web-тестов нет: …)» — все прогоны падали на разборе файла, тесты на GitHub не выполнялись; имя взято в кавычки, `push` с явным `branches: ['**']` для «Wait for CI» Railway; `pnpm audit` чинился overrides `multer >=2.3.0` |
| 17.09 | Админ без стороны | Решения владельца: администратор инстанса (`ADMIN_EMAILS`) — скрытая роль «Администратор», не сторона; регистрация на бете открыта (`REGISTRATION_MODE=open` — переменная Railway, кода нет); право создавать проекты по-прежнему выдаёт администратор. API: при регистрации администратора сторона не сохраняется (DTO требует её от всех, сервер игнорирует), `toAuthUser` и `PATCH /auth/profile` отдают `preferredRole: null`; тест в `accounts.spec`. SPA: `preferredRole: Side \| null`, `homeUrl` без проектов → `/admin` для администратора, `ROLE_ADMIN` в шапке меню и профиле, переключатель стороны в профиле скрыт; ADR 006 дополнение, PROD-RAILWAY (`REGISTRATION_MODE`), COPY.md |
| 17.09 | A2 (Railway) | Деплой на Railway из `main` после зелёного CI — `docs/PROD-RAILWAY.md` (пошагово для не-devops: postgres `pgvector/pgvector:pg17` с томом, api с томом `/app/apps/api/storage` и `MIGRATE_ON_START=true`, web с публичным доменом, «Wait for CI», watch paths, откат, стоимость). nginx контейнера web параметризован env (`apps/web/nginx.conf.template` + `docker-entrypoint.d/16-rr-proxy-env.envsh`): `API_UPSTREAM`, `DNS_RESOLVER` (по умолчанию из resolv.conf), `REAL_IP_FROM`; имя api через `resolver` + переменную в `proxy_pass` (адреса Railway меняются на каждый деплой), `listen [::]:80`. `X-Forwarded-For` в api — один адрес после realip → `TRUST_PROXY_HOPS=1` во всех окружениях (прод-оверлей 2 → 1, PROD.md). `railway.json` не добавлен: новые сервисы Railway его не читают (cutoff 2026-12-01) — вместо него `.railway/railway.ts` (IaC, применяется CLI, справочно). Compose-варианты не изменились по смыслу: `config -q` зелёный, стенд пересобран |
| 18.09 | A3 (бэкапы вне Railway), R-M3 | Копия в Backblaze B2 двумя механизмами, потому что том Railway монтируется только в один сервис. **База:** cron-сервис `backup` (`apps/backup`: `postgres:17-alpine` + rclone, `backup.sh`) — `pg_dump -Fc` → `s3:$B2_BUCKET/db/remarkround-ГГГГММДД-ЧЧММСС.dump` → сверка размера → чистка старше `BACKUP_KEEP_DAYS=30`, но не меньше 7 последних; режимы `restore-test` (скачать свежий и прочитать `pg_restore --list`), `restore <объект> --yes-i-know [--force]` (отказ при непустой `"User"`, одна транзакция), `list`, `prune`; cron `0 21 * * *` (02:00 Алматы), описан в `.railway/railway.ts`. **Файлы:** модуль `apps/api/src/offsite` — задача `offsite_copy` в общей очереди (старт + раз в сутки, как чистки R-M2): `rclone copy STORAGE_DIR b2:<бакет>/storage --immutable --size-only --min-age 1m`, секреты только через окружение дочернего процесса; `OFFSITE_B2_*` в `config.ts` — все четыре или ни одной; `/health` → `offsite: on\|off`; rclone добавлен в образ api. Проверено на стенде с MinIO вместо B2: бэкап → размер совпал → `restore-test` → `restore` в пустую базу (9 users / 20 remarks, `vector`, HNSW и триггеры на месте) → чистка на фальшивых старых дампах; api — tsc, спеки offsite/config/health/jobs, реальный rclone из образа api (новое/повтор/изменённый файл). CI: `sh -n`, shellcheck и сборка образа `backup`. Runbook — `docs/PROD-RAILWAY.md`, шаг 9 (Backblaze по кликам, таблица переменных, проверка, репетиция restore, порядок при аварии). **Владельцу:** аккаунт и приватный бакет B2, ключ, переменные в двух сервисах, сервис `backup` с Cron Schedule, первая репетиция restore с датой в таблице шага 9 |
