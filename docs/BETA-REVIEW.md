...
| 17.09 | 3.2 | Шаблон журнала — только `GET /imports/template.*` (фронт качает blob через `ApiService.importTemplate`, статика из `public/` и запись в неё из `make-journal-fixtures` удалены, спека сравнивает только API); удалён `GET /documents/:id` (вызовов нет); даты — `dateRu`/`dayMonthRu` из `format.ts`; `labels.contract.spec` сверяет ключи подписей и enum'ы web с Prisma по тексту исходников; scripts `make:fixtures`/`make:frames`/`make:screenshots`/`rag:eval` |
