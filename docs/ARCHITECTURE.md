...
| Invitations | Писем нет (ADR 013): незарегистрированного зовут ссылкой `/join/<token>`, зарегистрированный принимает или отклоняет в колокольчике (`/auth/invitations`); PM видит `inviteeName`. Ссылку смены пароля выдаёт администратор инстанса | `apps/api/src/{tenancy,projects,auth,admin}` |
...
Новый скрин → `DiffModule` → триплет old/new/diff в vision (пояснить, не закрыть) → interrupt `business` → только тогда `closed`. Если заказчик проверил исправление сам, он закрывает сразу после «Готово», без нового кадра и без графа (ADR 010); «Не исправлено» — только через ретест с кадром.
