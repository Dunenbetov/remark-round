...
- **Латентность.** Живой прогон evals 04.09 (`docs/EVALS.md`, `evals/results/2026-09-04-live-3.md`): триаж в среднем 5,0 с, ретест 1,3 с. Оба числа сняты мимо очереди.
- **Стоимость.** В тех же отчётах стоимость завышена: `gpt-4.1-mini` считалась по цене `gpt-4.1` — ошибка сравнения префиксов в `pricing.ts`, исправлена 18.09 (P3, `pricing.spec.ts`). У mini обе цены ровно в 5 раз ниже, поэтому ретест (только mini) завышен ровно в 5 раз, триаж — меньше (черновик идёт на `gpt-4.1`). Верные цифры по сценариям — после замеров M1. В `AgentRun.costUsd` входят вызовы chat-модели прогона, включая второй проход после «Не та цитата из ТЗ» и сбойные прогоны (`run-cost.spec.ts`). Не входят эмбеддинги, скидка за кэш, отменённые прогоны и вызовы до перезапуска процесса: учёт токенов живёт в памяти процесса, отмена его выбрасывает (`agent.service.ts:340-343`), а сметённый после рестарта прогон записывается без расхода.
...
Кэша ответов модели (semantic cache) нет сознательно. Каждое замечание уникально — свой текст, кадр, найденные разделы и соседи по раунду; доля попаданий была бы низкой (не мерили), а ошибка кэша — показать PM чужое обоснование. В доктрине это «stretch, не ценой ядра» (`REMARKROUND.md` §6.2). Автоматический кэш префикса промпта у OpenAI включается для промптов от 1024 токенов и только при точном совпадении начала (документация OpenAI, проверено 18.09). Общий префикс у нас — системный промпт со Skill; срабатывает ли кэш, не проверяли: поле `cached_tokens` мы не читаем и скидку в `costUsd` не учитываем (`pricing.ts:8-10`). Кэшировать эмбеддинг запроса нет смысла — порядка $0.00001. Что работает вместо кэша: документ индексируется один раз при загрузке, текст Skill читается в память один раз, на ретесте pixel-diff решает часть кадров без модели (4 из 13 кейсов A/B 04.09).
