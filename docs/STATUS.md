...
Писем нет (ADR 013 заменил ADR 009): очередь человека — журнал («ждут меня»). Статус ждёт роль: `awaiting_pm` → pm; `defect` (решение PM или «не исправлено» от бизнеса) → developer; `ready_for_retest` («Готово» разработчика — проверить: закрыть сразу или приложить новый кадр), `awaiting_business_close`, `cannot_tell`, `unspecified` (в документах нет ответа — решает заказчик) → business. В «ждут меня» руководителя приёмки кроме `awaiting_pm` попадает и `cannot_tell`: кадр может приложить и он.

Колокольчик указывает на эту очередь (ADR 016): о смене статуса сообщают правила по действию истории (`apps/api/src/notifications/audience.ts`) — «ждёт вас» тому, чья очередь пришла, «к сведению» заказчику о вехах; внутренние шаги модели (`triage`, `retest`, `cancel`, `run_failed`) — никому.
